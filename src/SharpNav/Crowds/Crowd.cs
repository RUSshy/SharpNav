			...
			for (int iter = 0; iter < 4; iter++)
			{
				...
				for (int i = 0; i < maxAgents; i++)
				{
					...
					if (agentAnims[i].T < ta)
					{
						...
						agents[i].Position = Vector3.Lerp(agentAnims[i].InitPos, agentAnims[i].StartPos, u);
					}
					else
					{
						float u = MathHelper.Normalize(agentAnims[i].T, ta, tb);
						agents[i].Position = Vector3.Lerp(agentAnims[i].StartPos, agentAnims[i].EndPos, u); ;
					}

                    agents[i].Vel = Vector3.Zero;
					agents[i].DesiredVel = Vector3.Zero;
				}
			}
		}
