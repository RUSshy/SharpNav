...
namespace SharpNav
{
    ...
    public static class MathHelper
	{
        ...
        public static int ClipPolygonToPlane(Vector3[] inVertices, Vector3[] outVertices, int numVerts, float planeX, float planeZ, float planeD)
		{
        ...
        public static int ClipPolygonToPlane(Vector3[] inVertices, Vector3[] outVertices, float[] distances, int numVerts, float planeX, float planeZ, float planeD)
		{
			...
			for (int i = 0, j = numVerts - 1; i < numVerts; j = i, i++)
			{
				...
				if (inj != ini)
				{
                    ...
                    outVertices[m] = (inVertices[i] - inVertices[j]) * s + inVertices[j];
					m++;
				}
