    ...
    public class NavTile : IEquatable<NavTile>
	{
				...
				for (int j = 0; j < numPolyVerts; j++)
				{
					...
					for (int k = 0; k < nei.Count; k++)
					{
						...
						if (dir == BoundarySide.PlusX || dir == BoundarySide.MinusX)
						{
							...
							link.BMin = (int)(Math.Clamp(tmin, 0.0f, 1.0f) * 255.0f);
							link.BMax = (int)(Math.Clamp(tmax, 0.0f, 1.0f) * 255.0f);
						}
		...
		public int QueryPolygons(BBox3 qbounds, List<NavPolyId> polys)
		{
			if (BVTree.Count != 0)
			{
				...
				PolyBounds b;
				float bminx = Math.Clamp(qbmin.X, tbmin.X, tbmax.X) - tbmin.X;
				float bminy = Math.Clamp(qbmin.Y, tbmin.Y, tbmax.Y) - tbmin.Y;
				float bminz = Math.Clamp(qbmin.Z, tbmin.Z, tbmax.Z) - tbmin.Z;
				float bmaxx = Math.Clamp(qbmax.X, tbmin.X, tbmax.X) - tbmin.X;
				float bmaxy = Math.Clamp(qbmax.Y, tbmin.Y, tbmax.Y) - tbmin.Y;
				float bmaxz = Math.Clamp(qbmax.Z, tbmin.Z, tbmax.Z) - tbmin.Z;
